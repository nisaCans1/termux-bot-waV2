...
* Author MhankBarBar, Farhan, Lord
		...
		case prefix+'daftar':
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case prefix+'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nama Bot* : LORD BOT\n➽ *Owner Bot* : wa.me/6283144780782\n➽ *prefix* : | ${prefix} |\n➽ *Total Block* : ${blocked.length}\n➽ *Aktif Sejak* : ${kyun(uptime)}\n\n➽ *Total Pengguna* : ${user.length} User\n➽ *Instagram* : https://www.instagram.com/sanglord_\n➽ *Special Thanks To* :\n\n➽ Allah SWT \n➽ MhankBarBar\n➽ Nurutomo\n➽ Monurios\n➽Fxc7\n➽https://github.com/Fxc7`
         ...
         case prefix+'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('6283144780782@s.whatsapp.net', options, text, {quoted: mek})
...
// akhir fitur anime
           ...
           case 'imagecyberspace':
                ...
                if (isLimit(sender)) return reply(limitend(pushname2))
					...
					buff = await getBuffer(anu.result)
					itsmeiky.sendMessage(from, buff, image, {quoted: mek})
					break
           case 'imagegame':
			         // Fix Bug By Zaki				
                if (isBanned) return reply(mess.only.benned)    
...
//creator
				...
				reply(ind.wait())
				buff = await getBuffer(`http://lolhuman.herokuapp.com/api/toloserti?apikey=WEMPYGANSS&name=${gatauda}`, {method: 'get'})
				dms.sendMessage(from, buff, image, {quoted: mek})
				await limitAdd(sender)
				break
				case '8bit': // Update By  Zaki				
              if (!isUser) return reply(mess.only.userB)
                if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
                if (isBanned) return reply(mess.only.benned)
				...
				buff = await getBuffer(anu.result.url)
				dms.sendMessage(from, buff, image, {quoted: mek, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
				case 'textbyname':
				 // Update By Zaki				
				...
				dms.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'galaxtext':
				 // Update By Zaki				
		    ...
		    case prefix+'map':
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *LORD BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
		...
		case prefix+'happymod':
			...
			break
				...
				if (body.startsWith(`${prefix}${command}`)) {
                  reply(`Ngelawak Bang?, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
                  const iirrr = fs.readFileSync('./mp3/Play-Date-Melanie-Martinez-Cover-by-邢凯悦XKY.mp3')
                dms.sendMessage(from, iirrr, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
